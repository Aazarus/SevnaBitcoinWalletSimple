...
namespace DotNetWallet.QBitNinjaJutsus
{
  public static class QBitNinjaJutsus
  {
    ...
    public static Dictionary<Coin, bool> GetUnspentCoins(IEnumerable<ISecret> secrets)
    {
      ...
      {
        var destination = secret.PrivateKey.ScriptPubKey.GetDestinationAddress(Configuration.DefaultNetwork);

        var client = new QBitNinjaClient(Configuration.DefaultNetwork);
